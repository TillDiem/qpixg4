...
std::vector<double> *hit_end_t = nullptr;
std::vector<double> *hit_end_x = nullptr;  // mm
std::vector<double> *hit_end_y = nullptr;  // mm
std::vector<double> *hit_end_z = nullptr;  // mm
...
TTree* GetHitTree(string InputDirectory, string NEUTFile){
    char* NEUTInputFileName1;
   NEUTInputFileName1 = Form("%s/%s.root", InputDirectory.c_str(), NEUTFile.c_str());
   NEUTInputFile1 = new TFile(NEUTInputFileName1, "UPDATE");


  TTree *NEUTInputTree = (TTree*)NEUTInputFile1->Get("event_tree");
  int event = 0;


  NEUTInputTree->SetBranchAddress("event", &event);
  NEUTInputTree->SetBranchAddress("hit_end_t", &hit_end_t);
  NEUTInputTree->SetBranchAddress("hit_end_x", &hit_end_x);
  NEUTInputTree->SetBranchAddress("hit_end_y", &hit_end_y);
  NEUTInputTree->SetBranchAddress("hit_end_z", &hit_end_z);
